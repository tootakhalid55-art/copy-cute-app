...
-- Small patch: replace t.txn_kind with t.kind in the cash branch of get_statement.
-- We recreate the function with the same body but corrected column reference.
      ...
      SELECT COALESCE(SUM(
        CASE WHEN _account_kind='customer' THEN
          ...
          CASE WHEN d.kind::text IN ('purchase_invoice','debit_note') THEN d.grand_total
               WHEN d.kind::text = 'credit_note' THEN -d.grand_total
               WHEN d.kind::text = 'payment_voucher' THEN -d.grand_total ELSE 0 END
        END),0) INTO v_opening
    ...
    WITH movements AS (
      SELECT d.issue_date AS txn_date, d.kind::text AS doc_kind, d.id AS doc_id, d.doc_number,
             ...
             ELSE CASE WHEN d.kind::text IN ('purchase_invoice','debit_note') THEN d.grand_total
                       WHEN d.kind::text = 'credit_note' THEN d.grand_total ELSE 0 END END AS credit,
             d.currency
      FROM public.documents d
      ...
      SELECT NULL::DATE, 'opening'::TEXT, NULL::UUID, NULL::TEXT, 'Opening balance'::TEXT,
             ...
             CASE WHEN v_opening<0 THEN -v_opening ELSE 0 END::NUMERIC, 'SAR'::TEXT, TRUE
      UNION ALL
