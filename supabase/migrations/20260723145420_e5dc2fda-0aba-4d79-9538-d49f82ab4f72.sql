...
BEGIN
  ...
  INSERT INTO public.posting_events(
    org_id, event_type, source_module, source_document_id,
...
DECLARE
  ...
  v_lines JSONB; v_is_customer BOOLEAN; v_bank_code TEXT;
BEGIN
  IF NOT (public.has_org_role(_org,v_uid,'owner') OR public.has_org_role(_org,v_uid,'admin') OR public.has_org_role(_org,v_uid,'accountant')) THEN
    RAISE EXCEPTION 'forbidden'; END IF;
  ...
  INSERT INTO public.posting_events(org_id,event_type,source_module,source_document_id,journal_entry_id,payload,status,created_by,processed_at)
    VALUES(_org,'refund_created', CASE WHEN v_is_customer THEN 'AR' ELSE 'AP' END, v_source, v_je_id,
